function ClockSR(){
	...
	var position = {
		...
		update : function () {
			...
			position.sec.x = options.center.x + Math.cos( asec ) * options.radius * 0.83;
			position.sec.y = options.center.y + Math.sin( asec ) * options.radius * 0.83;
			amin = (time.m + (time.s + 0.001 * time.ms)/60) * Math.PI * 2 / 60 - Math.PI/2;
			position.min.x = options.center.x + Math.cos( amin ) * options.radius * 0.64;
			position.min.y = options.center.y + Math.sin( amin ) * options.radius * 0.64;
			ahrs = (time.h + (time.m + time.s/60)/60) * Math.PI * 2 / 12 - Math.PI/2;
			position.hrs.x = options.center.x + Math.cos( ahrs ) * options.radius * 0.52;
			position.hrs.y = options.center.y + Math.sin( ahrs ) * options.radius * 0.52;
		}
	};//*/	
	...
	var drawClock = function () {
		ctx.save();
		ctx.strokeStyle = "#221436";
		ctx.arc(options.center.x,options.center.y,options.radius,0,Math.PI*2);
		ctx.lineWidth = "20";
		ctx.stroke();
		ctx.closePath();
		// drawMainTicks();
